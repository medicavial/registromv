...
if($funcion == 'registra'){
    ...
    $sql="Insert into Expediente (
                        ...
                        Pro_clave,
                        Uni_claveActual)
            VALUES(:folio,:prefijo,:cons,:uni_clave,:usu_registro,:apPat,:apMat,:nombre,:nomComp,:fecNa,:fecNa1,:edad,:meses,:mail,:telefono,:codPostal,:Exp_fecreg,:ciaClave,:proClave,:Uni_claveActual)";
...
/***************************                                                *************************************/
    if($datoFol['Cia_clave']==19 && $datoFol['Zon_clave']==6){

    $conexion = conectarMySQL();
    $query="Select Uni_zima from UnidadMedZim where Uni_mv=".$datoFol['Uni_clave'];
    ...
    $query="select Exp_poliza, Exp_siniestro, Exp_reporte, Exp_paterno, Exp_materno, Exp_nombre, Exp_completo, RIE_clave, Exp_RegCompania, Usu_registro, Cia_clave from Expediente where Exp_folio='".$fol."'";
    $result=$conexion->query($query);
 ...
 if ($rie==-1){$rie=7;}
    ...
    $stmt = $conexionZima->prepare($query);                                  
            ...
            $stmt->bindParam('REG_ajustador', $aju);   
            ...
            if ($stmt->execute()){ 
                ...
                Insert into PURegistro (Reg_clave, Reg_folio, UNI_clave, UNI_reporto, ASE_clave, REG_poliza, REG_ajustador, REG_siniestro, REG_reporte, REG_orden, REG_apaterno, REG_amaterno, REG_nombre, REG_fecnac, REG_genero, REG_ocupacion, REG_edad, REG_tel1, REG_tel2, REG_observaciones, REG_carretera, REG_conciente, REG_cinturon, SEI_clave, REG_ambulancia, REG_embarazo, SUP_clave, USU_login, REG_desde, VEH_clave, REG_fechahora, REG_nombrecompleto, RIE_clave, REG_folioelectronico, REG_folioMV, Est_clave)
                                values (".$id.",'".$folioZ."',".$uniZ.", '".$reporta."', ".$aseZ.",'".$pol."','".$aju."','".$sin."','".$rep."','".$orden."','".$pat."','".$mat."','".$nom."','".$fecnac."','".$genero."','".$ocu."','".$txtedad."','".$tel1."','".$tel2."','".$obs."','".$carr."','".$conc."','".$cin."','11','".$amb."','".$emb."','".$sup."','".$usulog."','PMVautomatico', '".$veh."', now(),'".$nomc."',".$rie.",'".$RegCia."', '".$fol."',8);");
                $statement->execute();
             }catch(Exception $e){
...
if($funcion == 'listadoFolios'){
    ...
    $db = conectarMySQL();
    $uni = $_GET['uni'];
    $query="Select Exp_folio, Cia_nombrecorto, Date_Format(Exp_fecreg, '%d-%m-%Y') as Fecha, Exp_paterno, Exp_materno, Exp_nombre, Exp_obs, Exp_solCancela, Exp_cancelado From Expediente inner join Unidad on Expediente.Uni_claveActual=Unidad.Uni_clave inner join Compania on Expediente.Cia_clave=Compania.Cia_clave Where Expediente.Uni_claveActual=".$uni." order by Exp_folio desc LIMIT 0 , 30";
    $result = $db->query($query);
    $datosFolio = $result->fetchAll(PDO::FETCH_OBJ);
    echo json_encode($datosFolio);
    $db = null;    
...
if($funcion == 'listMedicSymio'){
    ...
    $db = conectarMySQL();
    $query="select DISTINCT clave_denominacion,Sym_denominacion from ".$tabla."
            inner join SymioCuadroBasico on ".$tabla.".Clave_producto=SymioCuadroBasico.Clave_producto
            where ".$tabla.".Clave_producto like 'MED%' and Stock>0";
    $result = $db->query($query);
    $listProcedimientos = $result->fetchAll(PDO::FETCH_OBJ);
    echo json_encode($listProcedimientos);
    $db = null;    
...
if($funcion == 'enviaConstancia'){
    ...
    $query ="Insert into ConstanciaMedica(Exp_folio, Cons_motivo, Cons_obs, Cons_fecha, Cons_hora, Usu_registro)
            ...
            $temporal->bindParam("Usu_registro", $usr);                 
         if ($temporal->execute()){
            $texto='Nos dirijimos a ustedes ya que uno de los lesionados direccionados por ustedes a MedicaVial ha solicitado se le extienda una constancia de atencion medica, externando los siguientes motivos: '.$motivo;
            if($observaciones){
            ...
            $contenido='<HTML>
                ...
                $mimemail->set_from("coordmed@medicavial.com.mx");
                //$mimemail->set_to("facparticulares@medicavial.com.mx");
                $mimemail->set_to("enriqueerick@gmail.com");
                //$mimemail->add_bcc("egutierrez@medicavial.com.mx");
